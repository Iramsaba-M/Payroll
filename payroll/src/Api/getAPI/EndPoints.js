// export const EMP_API = ' employees/all_employees';
export const EMP_API = ' employees/';
// export const CARDS_API = 'employee/api/card';
export const CARDS_API = 'cardData/';


// export const BASIC_DETAILS_API = 'employee/basic-details/';
// export const BASIC_DETAILS_API = 'employees';
export const SALARY_DETAILS_API = 'basicdetail';


export const BANK_DETAILS_API = 'employee/bank-details';


export const SALARY_DETAILS_POST_API = 'salary_details/generate_ctc';

export const SALARY_DETAILS_GET_API = 'salary_details/';




// export const DOCUMENTS_API = 'employee_data/';
export const DOCUMENTS_API = 'employee/document-details';


export const ADITIONAL_DETAILS_API = 'employee/additional-details';

export const BasicDetails_export = '/export/basic-details/';

export const SalaryDetails_export = '/export/SalaryDetails_export';

export const BankDetails_export = '/export/bank-details/';

export const Documents_export = '/export/documents/';

export const Additionaldetails_export = '/export/additional-details/';

export const BASIC_DETAILS_API = 'basicdetail';
