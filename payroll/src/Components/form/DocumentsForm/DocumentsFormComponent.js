    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="form-line flex mb-4 ml-20">
        {config.slice(0,2).map((field, index) => (
          <div key={index}>
            <label className={DemoStyles[field.textcss].label}>{field.label}</label>
            {field.type === 'text' && (
              <TextComponent
                name={field.label}
                placeholder={field.placeholder}
      ...
      <div>
      <button type="button"   onClick={addCustomComponent} className='bg-gray-200 text-blue-600 p-2 px-4 rounded flex items-center ml-20 mb-2 mr-2'>+ Add Another Document</button>
      </div>
